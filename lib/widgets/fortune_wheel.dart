...
import 'package:ucspin/widgets/wheel_result_indicator.dart';
import 'package:ucspin/widgets/wheel_slice.dart';
...
class _FortuneWheelState extends State<FortuneWheel> with SingleTickerProviderStateMixin {
  ...
  void _initiateAnimation() {
    ...
    _animationController.addListener(() {
      print(((widget.children.length) * (_animationController.value % 1)).floor());
  ...
  return  AnimatedBuilder(
      builder: (BuildContext context, Widget? child) {
        print( "asd" + ((_animationController.value * pi * 2) - (pi / (widget.children.length))).toString());
        return Transform.rotate(
          // origin: Offset(0, (size / 10) / 2),
